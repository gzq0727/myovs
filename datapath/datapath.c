...
void ovs_dp_process_packet(struct sk_buff *skb, struct sw_flow_key *key)
{
    ...
    printk(KERN_INFO "ovs_dp_process_packet:run into ovs_dp_process_packet %d\n",packet_counter += 1);
    ...
    printk(KERN_INFO "ovs_dp_process_packet:simulation not find flow in datapath\n");

    if (unlikely(!flow)) {
        struct dp_upcall_info upcall;
        int error;

		printk(KERN_INFO "ovs_dp_process_packet:look up flow in userspace %d\n",upcall_time += 1);
    ...
    printk(KERN_INFO "ovs_dp_process_packet:find flow in datapath %d\n",dp_time += 1);
...
int ovs_dp_upcall(struct datapath *dp, struct sk_buff *skb,
    ...
    printk(KERN_INFO "ovs_dp_upcall: run into ovs_dp_upcall\n");
	...
	if (!skb_is_gso(skb)) {
		printk(KERN_INFO "ovs_dp_upcall: queue_userspace_packet\n");
		err = queue_userspace_packet(dp, skb, key, upcall_info, cutlen);
	} else {
		printk(KERN_INFO "ovs_dp_upcall: queue_gso_packet\n");
