...
int upcall_handler_time = 0;

int action_run_time = 0;
...
udpif_upcall_handler(void *arg)
{
    ...
    FILE *fp;
    fp = fopen("/home/gzq/flowlet_log23.txt","a+");

    while (!latch_is_set(&handler->udpif->exit_latch)) {
        if (recv_upcalls(handler)) {

            fprintf(fp,"udpif_upcall_handler %d\n",upcall_handler_time += 1);
            fflush(fp);
...
{
    FILE *fp;
    fp = fopen("/home/gzq/flowlet_log20.txt","a+");
    fprintf(fp,"run into upcall_xlate\n");
    fflush(fp);
    ...
    if (upcall->type == DPIF_UC_MISS) {
        ...
        fprintf(fp,"upcall->type is DPIF_UC_MISS\n");
        fflush(fp);
    ...
    //fprintf(fp,"before run xlate_actions action len %32u\n",xin.rule->up.actions->ofpacts_len);
    //fflush(fp);

    xlate_actions(&xin, &upcall->xout);


    fprintf(fp,"run xlate_actions %d\n\n",action_run_time += 1);
    fflush(fp);
    fclose(fp);
               ...
               struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    FILE *fp;
    fp = fopen("/home/gzq/flowlet_log21.txt","a+");
    fprintf(fp,"run into process_upcall\n");
    fflush(fp);


    switch (upcall_type) {
    case MISS_UPCALL:
        fprintf(fp,"upcall type is miss_upcall\n");
        fflush(fp);
