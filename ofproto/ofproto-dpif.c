...
{
    FILE *fp;
    fp = fopen("/home/gzq/flowlet_log15.txt", "a+");
    fprintf(fp,"ofproto_dpif_exectue_actions\n");
    fflush(fp);
    fclose(fp);
...
{
    FILE *fp;
    fp = fopen("/home/gzq/flowlet_log18.txt", "a+");
    fprintf(fp,"run rule_dpif_lookup_from_table");
    fflush(fp);
    ...
    if (flow->nw_frag & FLOW_NW_FRAG_ANY
        ...
        fprintf(fp,"flow->nw_frag & FLOW_NW_FRAG_ANT\n");
        fflush(fp);

        if (ofproto->up.frag_handling == OFPUTIL_FRAG_NORMAL) {
            /* We must pretend that transport ports are unavailable. */

            fprintf(fp,"OFPUTIL_FRAG_NORMAL\n");
            fflush(fp);
            ...
            fprintf(fp,"drop_frags_rule\n");
            fflush(fp);

            rule = ofproto->drop_frags_rule;
            if (stats) {

                fprintf(fp,"stats is not NULL\n");
                fflush(fp);
         ...
         next_id++, next_id += (next_id == TBL_INTERNAL))
    {
        *table_id = next_id;
        ...
        fprintf(fp,"run rule_dpif_lookup_in_table\n");
        fflush(fp);
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    FILE *fp;
    fp = fopen("/home/gzq/flowlet_log14.txt", "a+");
    fprintf(fp,"run packet_xlate\n");
    fflush(fp);
    ...
    if (xlate_actions(&xin, &xout) != XLATE_OK) {
        ...
        fprintf(fp,"prepare learn actions\n");
        fflush(fp);
